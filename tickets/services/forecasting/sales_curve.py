...
class SalesCurveCalculator:
    ...
    def get_conversion_series(self, event: Event) -> dict:
        ...
        the caller plots the raw *daily* conversion rate client-side (that day's
        orders ÷ views): a conversion rate is a ratio, so it can't be summed the way
        tickets or views can — the division happens per day in the browser.
