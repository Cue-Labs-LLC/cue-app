...
| `SMS_PASTE_MAX_RECIPIENTS` | Optional | Hard cap on how many phone numbers an organizer may paste into the composer's "Paste a list of numbers" audience at once (default 10000). Pasting more **blocks** the send (the organizer trims) rather than silently truncating. Pasted numbers are classified against the org's contacts by `classify_pasted_phones` (`tickets/services/sms_recipients.py`) into matched-subscribed / unsubscribed / unreachable / not-in-contacts; only matched, subscribed contacts become recipients (fed through the normal `manual_include_ids` path), and the breakdown is shown before sending |
...
| `SMS_SEND_RATE_PER_SEC` | Optional | Target outbound throughput per marketing-SMS campaign, in messages/second (default 5). `send_sms_campaign_task` staggers chunk dispatch (via Celery `countdown`) to hold ~this rate instead of blasting the whole audience at once — a burst from a low-trust-score sender gets carrier-filtered as spam (Twilio Error 30007). Keep low on a single sending number; raise only alongside a multi-number Messaging Service pool. At 5/s, 1,800 recipients finish in ~6 min |
| `SMS_CHUNK_SIZE` | Optional | Recipients per `send_sms_chunk_task` (default 10). Small chunks make the paced staggering smooth; each chunk still sends its recipients sequentially |
| `SMS_DAILY_SEGMENT_CAP` | Optional | Account-wide daily ceiling on SMS segments handed to carriers (default 2000), protecting the brand's T-Mobile A2P daily cap (shared across ALL orgs at a low trust score). A send that would push its send day past the cap is **blocked at compose time** (`finalize_campaign_send` raises `DailyCapExceededError`) so the organizer trims the recipient list — day-aware, measuring against the cap minus what's already sent today + already scheduled for that day (`daily_capacity_for` in `tickets/services/sms_limits.py`). A rare race the composer can't see (simultaneous confirms / a reschedule onto a full day) fails the campaign at send time with a refund and a `failure_reason`. Due/stuck campaigns dispatch soonest-linked-event first so a time-sensitive blast claims the budget over an evergreen one. Counts all segments conservatively (carrier of each recipient is unknown). `0` disables the guard |
| `SMS_BOUNCE_STRIKE_THRESHOLD` | Optional | How many **distinct** campaigns must fail a *transient* Twilio code (`30003` — handset off/out of coverage) against the same number before it's auto-suppressed as a bounce (default 3). Deliverability feedback loop: `tickets.sms.handle_delivery_failure` (called from both `send_sms_chunk_task` and `twilio_sms_status_webhook`) writes a global `PhoneSuppression(reason=BOUNCE)` so `SMSCampaign.materialize()` drops dead numbers from every future audience before they burn credits/daily-cap budget. **Hard**-bounce codes (`30005` unknown handset, `30006` landline/unreachable carrier, `21211` invalid To, `21614` not SMS-capable — `TWILIO_HARD_BOUNCE_ERROR_CODES`) suppress on the **first** failure regardless of this threshold; only the transient set (`TWILIO_TRANSIENT_FAIL_ERROR_CODES`) accrues strikes, so a temporary outage doesn't permanently blacklist a live number. Strikes are counted from existing `SMSMessageRecipient` rows (distinct `campaign_id`) — no stored counter. Deliberately does **not** suppress on `30007` (carrier spam-filtering = sender-reputation, not a dead number) or `21408` (geo-permission — handled by `sms_country_allowed`). Backfill past sends with `python manage.py suppress_sms_bounces [--apply]` (dry-run by default; scans our own recipient rows, no Twilio API). Opt-outs (`21610`) still suppress as `TWILIO_STOP` via the same helper + the separate `reconcile_sms_opt_outs` command |
| `SMS_ATTRIBUTION_WINDOW_DAYS` | Optional | Post-send conversion window (days, default 7) for **native (Cue) SMS** order/revenue attribution. A recipient's purchase of the campaign's linked event counts if it lands within this many days of their send. Last-touch across overlapping sends (most recent qualifying send wins → no double-count); refunded orders excluded; only event-linked campaigns attribute. Computed by `NativeSMSAttributionCalculator` (`tickets/services/marketing/sms_attribution.py`) → `SMSCampaign.attributed_orders`/`attributed_revenue`, recomputed after CSV import, direct checkout, and event-stats render. NULL until first computed (renders "—"); a concrete value incl. 0 afterward. (SlickText/external campaigns use the separate UTM-based `SMSAttributionCalculator` instead) |
...
| `WEBHOOK_DELIVERY_TIMEOUT` | Optional | Per-delivery HTTP timeout (seconds, default 10) for outbound general-purpose webhooks (`deliver_webhook_task`). Orgs register `WebhookEndpoint`s subscribed to domain events (`event.created`, `order.created`, `customer.created`); each fire enqueues a Celery task that POSTs an HMAC-SHA256-signed body and logs every attempt as a `WebhookDelivery` row. Signature covers `timestamp.event_type.delivery_id.body` (headers `X-Cue-Signature: t=…,v1=…`, `X-Cue-Event`, `X-Cue-Delivery-Id`, `X-Cue-Timestamp`); `delivery_id` is stable across retries so consumers dedupe at-least-once redeliveries. Retries on connection errors/timeouts/5xx; 4xx is terminal. Destination URLs are SSRF-validated (https-only outside DEBUG; private/loopback/reserved hosts blocked) at save and send time (`tickets/services/webhooks/validation.py`). Dispatch is via explicit `fire_*` calls at single-create sites in `tickets/services/webhooks/` — CSV bulk imports intentionally do not fire |
| `APNS_KEY_ID` | Push (iOS) | Apple APNs Auth Key (.p8) Key ID from the portal key detail page. Used to sign the ES256 provider JWT (`tickets/services/push_notifications/apns.py`) |
| `APNS_TEAM_ID` | Push (iOS) | Apple Developer Team ID (default `X6TZEUAVYR`); becomes the JWT `iss` claim |
| `APNS_BUNDLE_ID` | Push (iOS) | App bundle id / APNs topic (default `co.cueup.cue`); sent as the `apns-topic` header |
| `APNS_AUTH_KEY` | Push (iOS) | Contents of the .p8 private key (preferred on Render — store as a secret). If unset, `APNS_KEY_PATH` is read instead |
| `APNS_KEY_PATH` | Optional | Filesystem path to the .p8 key, used as a fallback when `APNS_AUTH_KEY` is unset |
| `APNS_USE_SANDBOX` | Optional | `True` (default) routes to `api.sandbox.push.apple.com` for dev builds; set `False` for App Store / TestFlight builds. Device tokens are environment-specific — a sandbox token won't deliver against production and vice versa |
| `PUSH_NOTIFICATION_DELIVERY_TIMEOUT` | Optional | Per-send HTTP timeout (seconds, default 10) for APNs pushes (`send_push_notification_task`). Organizers register a `DeviceToken` via `POST /api/notification/device-token/` (Token auth, one rotating token per organizer, org-scoped). Two payloads: a one-time §6.3 launch broadcast (`send_launch_push --confirm` management command, Apple's verbatim Value-Proposition copy) and a "Tap to Pay is ready" push fired server-side when Stripe's `account.updated` connect webhook reports `card_payments` active — guarded once-per-org by `Organization.tap_to_pay_enabled_push_sent` (admin action is the manual fallback). Stale tokens (BadDeviceToken/Unregistered/410) are deleted; network/timeout/429/5xx are retried. If APNs creds are unset the sender no-ops (safe gradual rollout). Requires the connected-account webhook endpoint to subscribe to `account.updated` in the Stripe dashboard |
| `LANGSMITH_TRACING` | Optional | Set `true` to send LangChain/LangGraph traces to LangSmith (https://smith.langchain.com) for observability — inspect each agent run, its tool calls, and token usage. Primarily for the SMS eval **agent judge** (`eval_sms_plans --agent-judge`, which builds a `create_react_agent` over the chat service's `build_tools`), but it is a **global** toggle so it also traces the chat agent and strategist generation. Unset/`false` = nothing sent, no overhead. Sends prompt + tool-result data to LangSmith's cloud, so leave off where that data must not leave the box. Read from `.env` via `load_dotenv()`; see `.env.example` |
| `LANGSMITH_API_KEY` | With tracing | LangSmith API key (create under Settings at smith.langchain.com). Required when `LANGSMITH_TRACING=true` |
| `LANGSMITH_PROJECT` | Optional | LangSmith project name that runs are grouped under (default `default`; this repo uses `sms-plan-eval`). Auto-created on first trace |
